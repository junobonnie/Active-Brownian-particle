# -*- coding: utf-8 -*-
"""
Created on Thu Jan  4 22:05:02 2024
...
class Simulator(Simulator):
    ...
    def main(self):
        ...
        for atom in self.world.atoms:
            ...
            new_v = mu_1*u + (mu_1-mu_2)*u.dot(atom_acc)*u + mu_2*atom_acc + c1*random.gauss(0, 1)*u + c2*SO2(m.pi/2).dot(u)
            ### 상호작용 추가 ###
            v_.append(new_v)
        ...
        count = 0
        for atom in self.world.atoms:
            atom.pos = Vector((x_[count].x+self.render.width/2)%(self.render.width)-self.render.width/2,
                              (x_[count].y+self.render.height/2)%(self.render.height)-self.render.height/2)
            atom.vel = v_[count]
            count = count + 1
#=============== 새로 추가된 부분 ===============#
...
# wall1 = Wall(800, 50, 0, Vector(-400, -200), blue)
...
walls = []
atoms = []
for i in range(-24*2,24*2):
    for j in range(-18*2,18*2):
        if random.random() < 1:
            position =  Vector(5*i, 5*j)
...
while True:
    ...
    # simulator.save_screen(저장위치, 건너뛸 개수(디폴트 0))
    if t > 800:
        simulator.save_snapshot('snapshots/')
        break
    ### 매시간 마다 시뮬레이션 스냅샷을 저장하는 코드 ###
